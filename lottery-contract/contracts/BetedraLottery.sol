...
import "./interfaces/IBetedraLottery.sol";
import "./interfaces/IWHBAR.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "./PrngSystemContract.sol";
import "hardhat/console.sol";

/** @title BetedraLottery Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BetedraLottery is ReentrancyGuard, Ownable {
    ...
    uint256 public maxPriceTicketInWHbar = 500 * 1e8; //1 HBAR = 100_000_000 Tinybars
    uint256 public minPriceTicketInWHbar = 0.05 * 1e8; //1 HBAR = 100_000_000 Tinybars
    ...
    IWHBAR public whbarToken;
    PrngSystemContract public randomGenerator;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInWHbar;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] hbarPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInWHbar;
        uint32 finalNumber;
    }
     ...
     * @param _whbarTokenAddress: address of the HBAR token
    ...
    constructor(
        ...
        whbarToken = IWHBAR(_whbarTokenAddress);
        randomGenerator = PrngSystemContract(_randomGeneratorAddress);
    ...
    function buyTickets(
        ...
        // Calculate number of HBAR to this contract
        uint256 amountWHbarToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInWHbar,
            _ticketNumbers.length
        );

        require(
            msg.value >= amountWHbarToTransfer,
            "Not enough to pay for tickets"
        );

        uint256 change = msg.value - amountWHbarToTransfer;
        if (change > 0) {
            (bool ok, ) = msg.sender.call{value: change}("");
            require(ok, "Refund failed");
        }
        whbarToken.deposit{value: amountWHbarToTransfer}(); // wrap HBAR ➜ WHBAR

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInWHbar += amountWHbarToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInWHbarToTransfer
        uint256 rewardInWHbarToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInWHbarToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        whbarToken.withdraw(rewardInWHbarToTransfer);

        (bool sent, ) = msg.sender.call{value: rewardInWHbarToTransfer}("");

        require(sent, "Transfer failed");

        emit TicketsClaim(
            msg.sender,
            rewardInWHbarToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function closeLottery(
        ...
        // Request a random number from the generator, lo , hi
        randomGenerator.getPseudorandomNumber(1000000, 1999999);

        randomResult = randomGenerator.getNumber();
    ...
    /**
     * @notice Draw the final number, calculate reward in HBAR per group, and make lottery claimable
     * @param _lotteryId: lottery id
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInWHbar) *
        ...
        // Calculate prizes in HBAR for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].hbarPerBracket[j] =
                ...
                // A. No HBAR to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].hbarPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInWHbar - amountToShareToWinners);

        // Transfer WHBAR to treasury address
        whbarToken.transfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in HBAR token
    ...
    function injectFunds(
        ...
        whbarToken.transferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInWHbar += _amount;
     ...
     * @param _priceTicketInWHbar: price of a ticket in HBAR
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInWHbar >= minPriceTicketInWHbar) &&
                (_priceTicketInWHbar <= maxPriceTicketInWHbar),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInWHbar: _priceTicketInWHbar,
            ...
            amountCollectedInWHbar: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(
        ...
        require(_tokenAddress != address(whbarToken), "Cannot be WHBAR token");
    ...
    /**
     * @notice Set HBAR price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInWHbar: minimum price of a ticket in HBAR
     * @param _maxPriceTicketInWHbar: maximum price of a ticket in HBAR
     */
    function setMinAndMaxTicketPriceInWHbar(
        uint256 _minPriceTicketInWHbar,
        uint256 _maxPriceTicketInWHbar
    ) external onlyOwner {
        require(
            _minPriceTicketInWHbar <= _maxPriceTicketInWHbar,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInWHbar = _minPriceTicketInWHbar;
        maxPriceTicketInWHbar = _maxPriceTicketInWHbar;
    }
     ...
     * @param _priceTicket price of a ticket (in HBAR)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].hbarPerBracket[_bracket];
        } else {
            return 0;
        }
    }
