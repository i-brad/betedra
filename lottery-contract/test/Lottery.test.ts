...
const BetedraLottery = artifacts.require("./BetedraLottery.sol");

const PRICE_HBAR = 400;

function gasToHbar(gas: number, gwei: number = 5) {
  const num = gas * gwei * 10 ** -9;
  return num.toFixed(4);
}

function gasToUSD(gas: number, gwei: number = 5, priceWHBAR: number = PRICE_HBAR) {
  const num = gas * priceWHBAR * gwei * 10 ** -9;
...
contract("Lottery V2", ([alice, bob, carol, david, erin, operator, treasury, injector]) => {
  ...
  // Contracts
  let lottery, mockwhbar, randomNumberGenerator;
  ...
  before(async () => {
    // Deploy mockwhbar
    mockwhbar = await MockERC20.new("Mock WHBAR", "WHBAR", _totalInitSupply);

    // Deploy MockRandomNumberGenerator
    randomNumberGenerator = await MockRandomNumberGenerator.new({ from: alice });

    // Deploy BetedraLottery
    lottery = await BetedraLottery.new(mockwhbar.address, randomNumberGenerator.address, { from: alice });
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("Users mint and approve WHBAR to be used in the lottery", async () => {
      for (let thisUser of [alice, bob, carol, david, erin, injector]) {
        await mockwhbar.mintTokens(parseEther("100000"), { from: thisUser });
        await mockwhbar.approve(lottery.address, parseEther("100000"), {
          from: thisUser,
        });
      }
    });
    ...
    it("Operator starts lottery", async () => {
      ...
      console.info(
        `        --> Cost to start the lottery: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
    ...
    it("Bob buys 100 tickets", async () => {
      ...
      console.info(
        `        --> Cost to buy the first 100 tickets: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );

      expectEvent.inTransaction(result.receipt.transactionHash, mockwhbar, "Transfer", {
        from: bob,
        to: lottery.address,
    ...
    it("Carol buys 1 ticket", async () => {
      ...
      console.info(
        `        --> Cost to buy a stand-alone ticket: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("David buys 10 tickets", async () => {
      ...
      console.info(
        `        --> Cost to buy 10 tickets: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("Owner does 10k WHBAR injection", async () => {
      ...
      console.info(
        `        --> Cost to do injection: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("Operator closes lottery", async () => {
      ...
      console.info(
        `        --> Cost to close lottery: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    });
    ...
    it("Numbers are drawn (9/9/9/9/9/9)", async () => {
      ...
      console.info(
        `        --> Cost to draw numbers (w/o ChainLink): ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    });
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 WHBAR collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 WHBAR
      ...
      console.info(
        `        --> Cost to claim ticket: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 WHBAR collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 WHBAR
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 WHBAR
      ...
      console.info(
        `        --> Cost to claim 9 tickets: ${gasToHbar(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    describe("Role exceptions", async () => {
      it("Owner can recover funds only if not WHBAR token", async () => {
        // Deploy Random Token
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockwhbar.address, parseEther("1"), { from: alice }),
          "Cannot be WHBAR token"
        );
      });
      ...
      it("Only owner/injector can call owner functions", async () => {
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockwhbar.address, parseEther("10"), { from: operator }),
          "Ownable: caller is not the owner"
        );
