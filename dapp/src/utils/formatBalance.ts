...
/**
 * Take a formatted amount, e.g. 15 HBAR and convert it to full decimal value, e.g. 15000000000000000
 */
...
export const getFullDisplayBalance = (
  ...
  const stringNumber = getBalanceAmount(balance, decimals).toFixed(
    displayDecimals as number
  );

  return displayDecimals
    ? _trimEnd(_trimEnd(stringNumber, "0"), ".")
    : stringNumber;
};
 ...
 * Note: rounds
 */
export const formatBigIntToFixed = (
  number: bigint,
  displayDecimals = 18,
  decimals = 18
) => {
